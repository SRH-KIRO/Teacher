...
class YoLov8TRT(object):
    ...
    def __init__(self, library, engine_file_path):
        ...
        self.engine = engine = runtime.deserialize_cuda_engine(serialized_engine)
    ...
    def Inference(self, img):
        ...
        for i in range(self.batch_size):
            result_boxes, result_scores, result_classid = self.post_process(output[i * 38001: (i + 1) * 38001], origin_h, origin_w)
        return result_boxes, result_scores, result_classid    
        #det_res = []
        #for j in range(len(result_boxes)):
        #    box = result_boxes[j]
        #    plot_one_box(box, img, label="{}:{:.2f}".format(self.categories[int(result_classid[j])], result_scores[j]),)
